...
const MsgBubble = React.memo(({ msg, isMe, username, isAdmin, onDelete, onSoftDelete, onReply, isNew, th, onStar, onCopy }) => {
  ...
  const bubbleStyle = {
    // maxWidth limits width; overflowWrap ensures whole words wrap, never mid-character
    maxWidth:'78%', overflowWrap:'break-word', wordWrap:'break-word',
    wordBreak:'normal',  // do NOT use 'break-word' here — it splits chars
    lineHeight:1.55,
    ...
    ...(isImg||isVid ? {} : {
      background: isMe
        ? (isLight ? th.bubbleMe : 'transparent')
        : bubbleBg,
      backgroundImage: isMe && !isLight ? th.bubbleMe : 'none',
      border: !isMe ? `1px solid ${th.bubbleThemBorder}` : (isMe&&isLight ? `1px solid ${th.bubbleThem}55` : 'none'),
    ...
    >
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:6, flexDirection:isMe?'row-reverse':'row', maxWidth:'88%' }}>
        ...
        <div style={bubbleStyle}>
          ...
          {/* Text + timestamp — WhatsApp layout:
               the invisible spacer at the end of the text pushes the last line
               right so the timestamp floats naturally to bottom-right */}
          {msg.text && !isImg && !isVid && (
            <div style={{ position:'relative' }}>
              <p style={{
                margin:0, fontSize:14, color:textColor,
                lineHeight:1.55,
                // correct word-wrap: only break at spaces, never mid-char
                overflowWrap:'break-word', wordWrap:'break-word', wordBreak:'normal',
                whiteSpace:'pre-wrap',  // honour newlines from shift+enter
              }}>
                {msg.text}
                {/* invisible spacer that reserves space for the timestamp */}
                <span style={{ display:'inline-block', width: isMe ? 68 : 50, height:0, verticalAlign:'bottom' }}/>
              </p>
              {/* timestamp absolutely positioned to bottom-right */}
              <div style={{
                position:'absolute', bottom:0, right:0,
                display:'flex', alignItems:'center', gap:3, paddingBottom:1
              }}>
          ...
          {msg.text && (isImg||isVid) && (
            <div style={{ padding:'6px 10px 4px' }}>
              <span style={{ fontSize:14, color:textColor, overflowWrap:'break-word', wordWrap:'break-word', wordBreak:'normal', whiteSpace:'pre-wrap' }}>{msg.text}</span>
            </div>
          )}
          {/* Timestamp for media messages */}
          {(isImg||isVid) && msg.text && (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:3, padding:'2px 10px 6px' }}>
              <span style={{ fontSize:10, color: isMe&&!isLight ? 'rgba(255,255,255,.55)' : th.textMuted, whiteSpace:'nowrap' }}>{getTime(msg.createdAt)}</span>
              {isMe && <DoubleCheck reads={reads} accentLight={th.accentLight} isLight={isLight}/>}
            </div>
          )}
          {(isImg||isVid) && !msg.text && (
            <div style={{ position:'absolute', bottom:8, right:8, display:'flex', alignItems:'center', gap:3,
              padding:'2px 7px', borderRadius:20, background:'rgba(0,0,0,.5)', backdropFilter:'blur(6px)' }}>
              <span style={{ fontSize:10, color:'#fff', whiteSpace:'nowrap' }}>{getTime(msg.createdAt)}</span>
...
export default function App() {
  ...
  /* ── INIT ── */
  useEffect(() => {
    // Note: Anonymous Auth may not be enabled in Firebase console.
    // We try it but don't block if it fails — Firestore rules are set to allow all writes.
    signInAnonymously(auth)
      .then(cred => { setFirebaseUser(cred.user); setAuthReady(true); })
      .catch(err => {
        console.warn('[auth] signInAnonymously failed:', err.code, '- proceeding without auth (rules allow all)');
        setAuthReady(true); // proceed anyway — rules allow if:true
      });
    onAuthStateChanged(auth, u => { if (u) { setFirebaseUser(u); setAuthReady(true); } });
    // Set authReady immediately so users can send messages without waiting
    setAuthReady(true);
  ...
  const sendMessage = async (e) => {
    ...
    if (sendingRef.current) return; // guard double-send
    ...
    try {
      ...
      console.log('[sendMessage] writing to:', msgsRef.path, '| room:', roomCode, '| user:', username);
      ...
      console.error('[sendMessage] FAILED:', err?.code, err?.message, err);
      toast$('Send failed: ' + (err?.code || err?.message || 'unknown error'), 'error');
